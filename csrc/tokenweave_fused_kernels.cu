...
__global__ void rms_norm_inplace_kernel(
    ...
    scalar_t* __restrict__ input,  // [..., hidden_size] — will be updated
                                   // in-place
  ...
  for (int idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
    ...
    input[blockIdx.x * hidden_size + idx] =
        ((scalar_t)(x * s_variance)) * weight[idx];  // normalize in-place
  }
}
...
void rms_norm_inplace(torch::Tensor& out,     // [..., hidden_size]
  ...
  VLLM_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "rms_norm_inplace_kernel", [&] {
...
#define LAUNCH_FUSED_RS_LN_AG_CTA(width)                                    \
  VLLM_BF16_DISPATCH_FLOATING_TYPES(                                        \
      input.scalar_type(), "fused_rs_ln_ag_cta_kernel", [&] {               \
        vllm::fused_rs_ln_ag_cta_kernel<scalar_t, width>                    \
            <<<grid, block, 0, stream>>>(                                   \
