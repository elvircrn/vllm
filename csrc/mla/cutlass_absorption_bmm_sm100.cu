...
void cutlass_bmm_caller_sm100_fp8(
  ...
  typename GemmKernel::EpilogueArguments epilogue_args{
      Gemm::Epilogue::prepare_args(scale_a, scale_b),
      d_ptr, c_stride, d_ptr, d_stride};
...
// M > 256
using BmmGemm_Default = cutlass_3x_gemm_sm100_fp8<
    InType, OutType, c3x::ScaledEpilogue,
    Shape<_256, _128, _128>, Shape<_2, _2, _1>, KS, ES, false>;

// 64 < M <= 256
using BmmGemm_M256 = cutlass_3x_gemm_sm100_fp8<
    InType, OutType, c3x::ScaledEpilogue,
    Shape<_128, _128, _128>, Shape<_2, _1, _1>, KS, ES, false>;

// M <= 64
using BmmGemm_M64 = cutlass_3x_gemm_sm100_fp8<
    InType, OutType, c3x::ScaledEpilogue,
    Shape<_64, _64, _128>, Shape<_1, _1, _1>, KS, ES, false>;

// M <= 16
using BmmGemm_M16 = cutlass_3x_gemm_sm100_fp8<
    InType, OutType, c3x::ScaledEpilogue,
    Shape<_128, _32, _128>, Shape<_1, _1, _1>, KS, ES, false>;
...
void mla_absorption_bmm(
  ...
  TORCH_CHECK(a.dtype() == torch::kFloat8_e4m3fn,
              "a must be fp8_e4m3, got ", a.dtype());
  TORCH_CHECK(b.dtype() == torch::kFloat8_e4m3fn,
              "b must be fp8_e4m3, got ", b.dtype());
  ...
  TORCH_CHECK(a.size(0) == b.size(0),
              "batch (L) mismatch: ", a.size(0), " vs ", b.size(0));
  TORCH_CHECK(a.size(2) == b.size(2),
              "K mismatch: ", a.size(2), " vs ", b.size(2));
  TORCH_CHECK(out.size(0) == a.size(1),
              "M mismatch: out[0]=", out.size(0), " vs a[1]=", a.size(1));
  TORCH_CHECK(out.size(1) == a.size(0),
              "L mismatch: out[1]=", out.size(1), " vs a[0]=", a.size(0));
  TORCH_CHECK(out.size(2) >= b.size(1),
              "out cols ", out.size(2), " must be >= N=", b.size(1));
