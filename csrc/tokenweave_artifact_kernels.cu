...
fused_rs_ln_cta_kernel(
    ...
    scalar_t* __restrict__ residual,      // [..., hidden_size]
    const scalar_t* __restrict__ weight,  // [hidden_size]
    uint32_t** signal_pads, size_t rank, size_t world_size, const float epsilon,
...
#pragma unroll
  for (int iter = 0; iter < tokens_per_iter; iter++) {
    ...
    for (int idx = tid; idx < vec_hidden_size; idx += bdimx) {
      auto mtemp =
          multimem_ld_reduce_add<16>(mcptr + offset_scalar + idx * width);
...
template <typename scalar_t, int width>
__global__ std::enable_if_t<(width > 0) && _typeConvert<scalar_t>::exists>
ar_cta_kernel(scalar_t* __restrict__ mcptr,  // [..., hidden_size] multimem_ptr
              uint32_t** signal_pads, size_t rank, size_t world_size,
              const int num_tokens, const int hidden_size) {
  // Check vectorization assumptions
  static_assert(std::is_pod_v<_f16Vec<scalar_t, width>>);
  static_assert(sizeof(_f16Vec<scalar_t, width>) == sizeof(scalar_t) * width);
...
#pragma unroll
  for (int iter = 0; iter < tokens_per_iter; iter++) {
    ...
    int offset_scalar = token_id * hidden_size;
    // First pass: add + accumulate sum of squares
    for (int idx = tid; idx < vec_hidden_size; idx += bdimx) {  // all reduce
      auto mtemp =
          multimem_ld_reduce_add<16>(mcptr + offset_scalar + idx * width);
...
template <typename scalar_t, int width>
__global__ std::enable_if_t<(width > 0) && _typeConvert<scalar_t>::exists>
rs_cta_kernel(scalar_t* __restrict__ input,  // [..., hidden_size]
              scalar_t* __restrict__ mcptr,  // [..., hidden_size] multimem_ptr
              uint32_t** signal_pads, size_t rank, size_t world_size,
              const int num_tokens, const int hidden_size) {
  // Check vectorization assumptions
...
template <typename scalar_t, int width>
__global__ std::enable_if_t<(width > 0) && _typeConvert<scalar_t>::exists>
ag_cta_kernel(scalar_t* __restrict__ input,  // [..., hidden_size]
              scalar_t* __restrict__ mcptr,  // [..., hidden_size] multimem_ptr
              uint32_t** signal_pads, size_t rank, size_t world_size,
              const int num_tokens, const int hidden_size) {
  // Check vectorization assumptions
...
#define LAUNCH_FUSED_ADD_RMS_NORM_CTA(width)                                   \
  VLLM_DISPATCH_FLOATING_TYPES(                                                \
      input.scalar_type(), "fused_add_rms_norm_cta_kernel", [&] {              \
        vllm::fused_add_rms_norm_cta_kernel<scalar_t, width>                   \
...
void fused_add_rms_norm_cta(torch::Tensor& input,     // [..., hidden_size]
  ...
  dim3 block(std::min(
      1024, (hidden_size / 8 + 31) / 32 * 32));  // match kernel assumptions
  ...
  if (ptrs_are_aligned && hidden_size % 8 == 0) {
    ...
    TORCH_CHECK(false,
                "Input, residual, and weight tensors must be 16-byte aligned "
                "and hidden_size must be divisible by 8 for optimized kernel.");
  }
}
...
#define LAUNCH_FUSED_RS_LN_CTA(width)                                       \
  VLLM_BF16_DISPATCH_FLOATING_TYPES(                                        \
      input.scalar_type(), "fused_rs_ln_cta_kernel", [&] {                  \
        vllm::fused_rs_ln_cta_kernel<scalar_t, width>                       \
            <<<grid, block, 0, stream>>>(                                   \
                input.data_ptr<scalar_t>(),                                 \
                reinterpret_cast<scalar_t*>(mcptr),                         \
                residual.data_ptr<scalar_t>(), weight.data_ptr<scalar_t>(), \
                reinterpret_cast<uint32_t**>(signal_pads),                  \
                static_cast<size_t>(rank), static_cast<size_t>(world_size), \
...
#define LAUNCH_AR_CTA(width)                                              \
  VLLM_BF16_DISPATCH_FLOATING_TYPES(                                      \
      input.scalar_type(), "ar_cta_kernel", [&] {                         \
        vllm::ar_cta_kernel<scalar_t, width><<<grid, block, 0, stream>>>( \
            reinterpret_cast<scalar_t*>(mcptr),                           \
            reinterpret_cast<uint32_t**>(signal_pads),                    \
            static_cast<size_t>(rank), static_cast<size_t>(world_size),   \
            num_tokens, hidden_size);                                     \
...
void multimem_ar_cta(torch::Tensor& input,  // [..., hidden_size]
  ...
  if (hidden_size % 8 == 0) {
    ...
    TORCH_CHECK(false,
                "hidden_size must be divisible by 8 for optimized kernel.");
  }
}
...
#define LAUNCH_RS_CTA(width)                                                \
  VLLM_BF16_DISPATCH_FLOATING_TYPES(                                        \
      input.scalar_type(), "rs_cta_kernel", [&] {                           \
        vllm::rs_cta_kernel<scalar_t, width><<<grid, block, 0, stream>>>(   \
            input.data_ptr<scalar_t>(), reinterpret_cast<scalar_t*>(mcptr), \
            reinterpret_cast<uint32_t**>(signal_pads),                      \
            static_cast<size_t>(rank), static_cast<size_t>(world_size),     \
            num_tokens, hidden_size);                                       \
      });
void multimem_rs_cta(torch::Tensor& input,  // [..., hidden_size]
  ...
  if (ptrs_are_aligned && hidden_size % 8 == 0) {
    ...
    TORCH_CHECK(false,
                "Input tensors must be 16-byte aligned and hidden_size must be "
...
#define LAUNCH_AG_CTA(width)                                                \
  VLLM_BF16_DISPATCH_FLOATING_TYPES(                                        \
      input.scalar_type(), "ag_cta_kernel", [&] {                           \
        vllm::ag_cta_kernel<scalar_t, width><<<grid, block, 0, stream>>>(   \
            input.data_ptr<scalar_t>(), reinterpret_cast<scalar_t*>(mcptr), \
            reinterpret_cast<uint32_t**>(signal_pads),                      \
            static_cast<size_t>(rank), static_cast<size_t>(world_size),     \
            num_tokens, hidden_size);                                       \
...
#define LAUNCH_SIMPLE_FUSION_RS_LN_AG_CTA(width)                            \
  VLLM_BF16_DISPATCH_FLOATING_TYPES(                                        \
      input.scalar_type(), "simple_fusion_rs_ln_ag_cta_kernel", [&] {       \
        vllm::simple_fusion_rs_ln_ag_cta_kernel<scalar_t, width>            \
            <<<grid, block, 0, stream>>>(                                   \
                input.data_ptr<scalar_t>(),                                 \
                reinterpret_cast<scalar_t*>(mcptr),                         \
                residual.data_ptr<scalar_t>(), weight.data_ptr<scalar_t>(), \
