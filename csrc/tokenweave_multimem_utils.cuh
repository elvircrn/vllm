...
__device__ __forceinline__ uint32_t cas(uint32_t* addr, uint32_t compare,
                                        uint32_t val) {
...
template <MemOpSem Sem>
__device__ __forceinline__ bool try_put_signal(uint32_t* addr,
                                               size_t timeout_ms) {
...
template <MemOpSem Sem>
__device__ __forceinline__ bool try_wait_signal(uint32_t* addr,
                                                size_t timeout_ms) {
  size_t deadline = global_timer_ns() + timeout_ms * ns_per_ms;
...
template <MemOpSem Sem>
__device__ __forceinline__ void sync_remote_blocks(uint32_t** signal_pads,
                                                   size_t rank,
                                                   size_t world_size);
...
__device__ __forceinline__ void sync_remote_blocks<MemOpSem::Relaxed>(
  ...
  if (threadIdx.x < world_size) {
    ...
    put_signal<MemOpSem::Relaxed>(signal_pads[target_rank] +
                                  blockIdx.x * world_size + rank);
    wait_signal<MemOpSem::Relaxed>(signal_pads[rank] + blockIdx.x * world_size +
                                   target_rank);
  }
}
...
__device__ __forceinline__ void sync_remote_blocks<MemOpSem::AcqRel>(
  ...
  if (threadIdx.x < world_size) {
    auto target_rank = threadIdx.x;
    put_signal<MemOpSem::Release>(signal_pads[target_rank] +
                                  blockIdx.x * world_size + rank);
    wait_signal<MemOpSem::Acquire>(signal_pads[rank] + blockIdx.x * world_size +
                                   target_rank);
  }
}
      ...
      __device__ __inline__ Vec<Alignment> operator()(type* mc_ptr) {       \
        ...
        if constexpr (Alignment == 16) {                                    \
          asm("multimem.ld_reduce.relaxed.sys.global.add" acc_prec          \
              ...
              : "=r"(vec.u32[0]), "=r"(vec.u32[1]), "=r"(vec.u32[2]),       \
                "=r"(vec.u32[3])                                            \
              : "l"(mc_ptr)                                                 \
              : "memory");                                                  \
...
__device__ __inline__ void multimem_st(T* mc_ptr, Vec<Alignment>& vec) {
  ...
  if constexpr (Alignment == 16) {
    asm("multimem.st.relaxed.sys.global.v4.f32 [%0], {%1,%2,%3,%4};"
        :
        : "l"(mc_ptr), "r"(vec.u32[0]), "r"(vec.u32[1]), "r"(vec.u32[2]),
          "r"(vec.u32[3])
        : "memory");
  } else if constexpr (Alignment == 8) {
