...
class DeepEPV2PrepareAndFinalize(mk.FusedMoEPrepareAndFinalizeModular):
    ...
    **Prefill mode (use_cudagraph=False):**
    ...
    def _do_dispatch(
        ...
        # Fused MoE-align: decode only. align_m is the block expert_ids is padded
        # to here. It is chosen from a CPU-side token count (cudagraph-safe) and
        # RECORDED in the handle (moe_align_expert_ids / align_m below); the
        # experts must tile the GEMM at exactly this block M at apply time, or
        # they mis-stride expert_ids and read the wrong expert's weights. The
        # experts pin their tile to this recorded align_m (see
        # FusedHummingExperts.prepare_humming_moe_kwargs) rather than re-deriving
        # it from the padded recv buffer, so the two sides agree by construction.
