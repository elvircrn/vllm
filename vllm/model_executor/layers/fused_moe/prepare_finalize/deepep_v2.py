    ...
    **Prefill mode (use_cudagraph=False):**
    ...
    def _receiver(
        ...
        if self.use_cudagraph:
            # Decode mode: carry the device-side per-rank prefix sum so the
            # SiTU activation can skip the worst-case padding rows. There is no
            # CPU sync here, so recv_expert_num_tokens is empty and
            # expert_tokens_meta is None above; attach psum to a metadata
            # object whose per-expert counts stay unset (experts recover them
            # from topk_ids).
