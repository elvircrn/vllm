...
class HummingIndexedExperts(HummingExpertsBase):
    ...
    def _block_size_for_shape_m(self, valid_shape_m: int) -> int:
    ...
    def select_moe_block_size(self, topk_ids: torch.Tensor) -> int:
        ...
        This is the single source of truth for the alignment granularity: the
        DeepEP v2 fused dispatch (when enabled) queries it to set align_m so the
        dispatch-emitted grouped-GEMM metadata is padded to exactly this block,
        and `prepare_humming_moe_kwargs` uses it to select the tuning config.
        """
        return self._block_size_for_shape_m(self.estimate_local_valid_shape_m(topk_ids))
    ...
    def prepare_humming_moe_kwargs(
        ...
        # Fast path: the DeepEP v2 dispatch epilogue already produced the
        # grouped-GEMM metadata (sorted_ids / expert_ids / num_tokens_padded)
        # padded to align_m. Reuse it directly and skip the moe_align_block_size
        # kernel + the globalize/count_and_sort it folds in. The dispatch used
        # this same selector for align_m, so its padding matches the tile M.
        if (
            expert_tokens_meta is not None
            and expert_tokens_meta.sorted_token_ids is not None
        ):
            # The dispatch already padded the metadata to this block size (its
            # align_m), so it is authoritative for the GEMM tile M -- do not
            ...
            moe_block_size = self.select_moe_block_size(topk_ids)
