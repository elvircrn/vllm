...
if TYPE_CHECKING:
    ...
    """If set, the torch profiler performs a torch.distributed barrier across all
    ranks immediately before it starts capturing (on start_profile / after any
    delay). This synchronizes the capture window so per-rank traces from
    different nodes cover the same iterations, which is required to align
    multi-node profiles. Only enable when every rank is being profiled: it adds
    a collective on the profiler control path that will hang if a subset of
    ranks never receives start_profile."""
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # If set to 1, the torch profiler barriers across all ranks immediately
    # before it starts capturing, so multi-node per-rank traces cover the same
    # iterations. Only enable when every rank is being profiled.
