...
def _synchronize_dp_ranks(
    ...
    if profiler_sync is not None:
        ...
        # DEBUG (multinode profiler propagation): fires only during a profiling
        # window (this rank pending, or the OR-reduce carried a start bit), so it
        # cannot spam steady-state serving. Shows the actual reduce group size --
        # if reduce_group_size == 1 the DP all-reduce is per-engine and the start
        # bit cannot cross engines under external-LB.
